...
from .serializers import (
    PersonaSerializer, EstudianteSerializer, TutorSerializer, 
    MateriaSerializer, TutoriaSerializer
)
...
def persona_list(request):
    return render(request, 'gestion_tutorias/persona_list.html', {'personas': Persona.objects.all()})
...
def persona_create(request):
    ...
    return render(request, 'gestion_tutorias/persona_form.html', {'form': form})
...
def persona_delete(request, pk):
    ...
    if request.method == 'POST':
        persona.delete()
        return redirect('persona_list')
    return render(request, 'gestion_tutorias/confirm_delete.html', {'object': persona})

@login_required
def estudiante_list(request):
    return render(request, 'gestion_tutorias/estudiante_list.html', {'estudiantes': Estudiante.objects.all()})
...
def tutor_list(request):
    return render(request, 'gestion_tutorias/tutor_list.html', {'tutores': Tutor.objects.all()})
...
def materia_list(request):
    return render(request, 'gestion_tutorias/materia_list.html', {'materias': Materia.objects.all()})
...
def tutoria_list(request):
    return render(request, 'gestion_tutorias/tutoria_list.html', {'tutorias': Tutoria.objects.all()})

@login_required
def tutoria_create(request):
    form = TutoriaForm(request.POST or None)
    if form.is_valid():
        form.save()
...
def tutoria_edit(request, pk):
    ...
    form = TutoriaForm(request.POST or None, instance=tutoria)
    if form.is_valid():
        form.save()
        return redirect('tutoria_list')
...
def reporte_tutorias_estado(request):
    context = {
        'pendientes': Tutoria.objects.filter(estado='pendiente').count(),
        'aceptadas': Tutoria.objects.filter(estado='aceptada').count(),
        'finalizadas': Tutoria.objects.filter(estado='finalizada').count(),
        'rechazadas': Tutoria.objects.filter(estado='rechazada').count(),
        'total': Tutoria.objects.count(),
    }
    return render(request, 'gestion_tutorias/reporte_estado.html', context)
